      ...
      try {
        ...
        if (editingSequenceId) {
          ...
          if (sequenceData) {
            ...
            setFocus(sequenceData.focus);
            // Load peak poses from the database
            if (sequenceData.peak_poses) {
              setPeakPoses(sequenceData.peak_poses);
            } else {
              setPeakPoses([]);
            }
            setShowFilters(false); // Hide filters initially when editing
          }
        }
      } catch (error) {
  ...
  const generateSequence = async () => {
    ...
    try {
      ...
      const peakPosesNames = peakPoses.length > 0 
        ...
        : '';
      setTitle(`${duration}-Minute ${level} ${focus.join(' & ')} Flow${peakPosesNames}`);
    } catch (error) {
      ...
      if (editingSequenceId) {
        ...
        const { error: insertError } = await supabase
          ...
          .insert({
            ...
            name: title || `${focus.join(' & ')} ${level} Flow`,
            duration,
            level,
            focus,
            poses: sequence,
      ...
      >
        <div className="text-center space-y-4">
          <h1 className="text-4xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-400">
            {editingSequenceId ? 'Edit Your Flow' : 'Generate Your Flow'}
          </h1>
          <p className="text-xl text-gray-400">
            {editingSequenceId ? 'Modify your saved sequence' : 'Create a personalized yoga sequence'}
          </p>
        </div>
